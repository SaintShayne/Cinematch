'use client'

import Image from 'next/image'
import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { cn, formatRating, formatYear, truncate } from '../../lib/utils'
import { useAuth } from '../../lib/context/AuthContext'
import { useWatchlist } from '../../lib/hooks/useWatchlist'
import { useRecentlyViewed } from '../../lib/hooks/useRecentlyViewed'

export default function MovieCard({ movie, onSelect, showSave = true }) {
  const router = useRouter()
  const { user } = useAuth()
  const { isInWatchlist, addToWatchlist, removeFromWatchlist } = useWatchlist()
  const { trackView } = useRecentlyViewed()
  const [imgError, setImgError] = useState(false)
  const [saving, setSaving] = useState(false)

  // Use title as the stable key — our API does not return numeric IDs
  const inWatchlist = isInWatchlist(movie.title)

  const handleSelect = () => {
    trackView(movie)
    onSelect?.(movie)
  }

  const handleSave = async (e) => {
    e.stopPropagation()
    if (!user) {
      router.push(`/login?redirect=${encodeURIComponent(window.location.pathname)}`)
      return
    }
    setSaving(true)
    if (inWatchlist) {
      await removeFromWatchlist(movie.title)
    } else {
      await addToWatchlist(movie)
    }
    setSaving(false)
  }

  const posterSrc =
    !imgError && movie.poster_url ? movie.poster_url : null

  return (
    <div
      onClick={handleSelect}
      className={cn(
        'group relative rounded-xl overflow-hidden cursor-pointer',
        'bg-surface-elevated border border-[rgba(255,255,255,0.06)]',
        'transition-all duration-250 hover:border-[rgba(255,255,255,0.15)] hover:-translate-y-0.5 hover:shadow-elevated'
      )}
    >
      {/* Poster */}
      <div className="relative aspect-[2/3] overflow-hidden bg-surface">
        {posterSrc ? (
          <Image
            src={posterSrc}
            alt={movie.title}
            fill
            sizes="(max-width: 640px) 50vw, (max-width: 1024px) 25vw, 180px"
            className="object-cover transition-transform duration-300 group-hover:scale-105"
            onError={() => setImgError(true)}
          />
        ) : (
          <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-text-muted">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 opacity-30">
              <path d="M2 6a2 2 0 012-2h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm2 0v12h16V6H4zm4 2h8v2H8V8zm0 4h5v2H8v-2z" />
            </svg>
            <span className="text-2xs text-center px-2 leading-tight opacity-50">
              {truncate(movie.title, 30)}
            </span>
          </div>
        )}

        {/* Gradient overlay on hover */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200" />

        {/* Overview on hover */}
        {movie.overview && (
          <div className="absolute inset-0 flex items-end p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            <p className="text-2xs text-white/80 line-clamp-4 leading-relaxed">
              {movie.overview}
            </p>
          </div>
        )}

        {/* Save button — visible to all; guests are redirected to login */}
        {showSave && (
          <button
            onClick={handleSave}
            disabled={saving}
            aria-label={inWatchlist ? 'Remove from watchlist' : 'Add to watchlist'}
            className={cn(
              'absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center',
              'transition-all duration-150',
              inWatchlist
                ? 'bg-red text-white opacity-100'
                : 'bg-black/60 text-white opacity-0 group-hover:opacity-100 hover:bg-red/80'
            )}
          >
            <svg viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5">
              <path d="M6.75 2.75A2.75 2.75 0 004 5.5v11.75a.75.75 0 001.26.55L10 13.06l4.74 4.74A.75.75 0 0016 17.25V5.5A2.75 2.75 0 0013.25 2.75h-6.5z" />
            </svg>
          </button>
        )}
      </div>

      {/* Card info */}
      <div className="p-3">
        <p className="text-xs font-semibold text-text-primary truncate leading-tight">
          {movie.title}
        </p>
        <div className="flex items-center justify-between mt-1">
          <span className="text-2xs text-text-muted">
            {formatYear(movie.release_year || movie.release_date)}
          </span>
          {movie.vote_average != null && (
            <span className="text-2xs text-yellow-400 font-medium">
              ★ {formatRating(movie.vote_average)}
            </span>
          )}
        </div>
      </div>
    </div>
  )
}
